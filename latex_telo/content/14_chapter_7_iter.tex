...
\begin{algorithm}
	...
	\State \fun{initializeParameters}
	\For{$\mathrm{i} < \mathrm{iter}_{\mathrm{max}}$}
		...
		\State Evaluate \fun{gfxbndnorm} = $ || X ||_{\mathrm{L}^2(\Gamma_{\infty , \mathrm{n}})} $
		\If{\fun{gfxbndnorm} $<$ $\varepsilon$} 
			\State \fun{increaseScalingParameters}
		\If{\fun{parameterTooBig}}
			\State \fun{break}
		\EndIf
		\EndIf
